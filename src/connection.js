...
FTP.prototype.connect = function(options) {
  ...
  this.options.socksv5 = options.socksv5 || null;
  ...
  function onconnect() {
    ...
    function reentry(err, text, code) {
      ...
      if ((cmd === 'AUTH TLS' && code !== 234 && self.options.secure !== true)
          || (cmd === 'AUTH SSL' && code !== 234 && code !== 334)
          || (cmd === 'PBSZ' && code !== 200)
          || (cmd === 'PROT' && code !== 200)) {
        self.emit('error', makeError(code, 'Unable to secure connection(s): ' + text));
        return self._socket && self._socket.end();
      }
      ...
      if (!cmd) {
        ...
        // secureOptions.ciphers = 'ECDHE-RSA-AES256-SHA:AES256-SHA:RC4-SHA:RC4:HIGH:!MD5:!aNULL:!EDH:!AESGCM';
        ...
        socket.once('end', self._onend);
        socket.on('error', self._onerror);
      }
    }
  }

  if (this.options.socksv5 && this.options.socksv5.proxyHost && this.options.socksv5.proxyPort) {
    var proxyOpts = {
      proxyHost: this.options.socksv5.proxyHost,
      proxyPort: this.options.socksv5.proxyPort,
      host: this.options.host,
      port: this.options.port,
    };
    var proxyUser = this.options.socksv5.proxyUser;
    var proxyPort = this.options.socksv5.proxyPass;
    if (proxyUser && proxyPort) {
      proxyOpts.auths = [ socksv5.auth.UserPassword(proxyUser, proxyPort) ];
    } else {
      proxyOpts.auths = [ socksv5.auth.None() ];
    }

    var proxyClient = socksv5.connect(proxyOpts, function(ss) {
      socket = ss;
      self._socket = socket;
    ...
    proxyClient.once('close', function(had_err) {
      clearsocket();
      self._ondone();
...
FTP.prototype.setupSocket = function(socket) {
  var self = this;
  var secureOptions = this.options.secureOptions;
  var debug = this._debug;

  socket.setTimeout(0);
  socket.setKeepAlive(true);

  var noopreq = this._noopreq = {
        cmd: 'NOOP',
        cb: function() {
          clearTimeout(self._keepalive);
  ...
  var timer = self._timer = setTimeout(function() {
